...
# Function to update button positions on window resize
def update_button_positions(event=None):
    screen_width = account.winfo_width()
    btn2.place(x=screen_width - 50, y=0)  # Cancel button
    restore_button.place(x=screen_width - 100, y=0)  # Restore button
    btn.place(x=screen_width - 150, y=0)  # Minimize button
...
# Bind the <Configure> event to update button positions
account.bind("<Configure>", update_button_positions)
